...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .pill.dist{ cursor:pointer; user-select:none; }
    .pill.dist.has{
      color:rgba(255,255,255,.9);
      border-color: rgba(255,59,48,.55);
      background: rgba(255,59,48,.12);
    }
    .pill.dist:hover{ background: rgba(255,255,255,.08); }
    ...
    /* Clickable destination (vehicle overlay trigger) */
    .toLink{
      border:none;
    ...
    .toLink:hover{ text-decoration: underline; text-decoration-style: dotted; }
    .toLink:focus{ outline: 2px solid rgba(47,125,255,.5); outline-offset: 3px; }
    ...
    /* Vehicle overlay */
    .vehSummary{display:flex; gap:10px; flex-wrap:wrap; align-items:center;}
    .vehStops{margin-top:10px;}
    .vehRow{
      display:grid;
    ...
    .vehTimes{font-weight:900; letter-spacing:.3px;}
    .vehTimes .muted{font-weight:600}
    .vehStation{font-weight:900;}
    .vehSub{margin-top:4px; color: var(--muted); font-size:12px; line-height:1.25;}
    .vehRight{justify-self:end; text-align:right;}
    .plMini{
      display:inline-grid; place-items:center;
    ...
    .plMini .lbl{font-size:10px; color: rgba(255,255,255,.65); }
    .plMini .num{font-size:20px; font-weight:1000; margin-top:2px;}
    ...
    @media (max-width:720px){
      ...
      .vehRow{grid-template-columns: 84px 1fr 84px;}
    }
  </style>
</head>
<body>
  ...
  <!-- Vehicle overlay -->
  <div class="overlayBack" id="vehOverlayBack" aria-hidden="true">
    <div class="overlayCard" role="dialog" aria-modal="true" aria-label="Train details">
      <div class="overlayHead">
        <div class="overlayTitle" id="vehOverlayTitle">Train details</div>
        <button class="overlayClose" id="vehOverlayClose" aria-label="Close">✕</button>
      </div>
      <div class="overlayBody">
        <div class="muted" id="vehOverlaySub">Loading…</div>
        <div id="vehOverlayList" style="margin-top:10px;"></div>
      </div>
    </div>
  </div>
...
const vehOverlayBack = document.getElementById('vehOverlayBack');
const vehOverlayClose = document.getElementById('vehOverlayClose');
const vehOverlayTitle = document.getElementById('vehOverlayTitle');
const vehOverlaySub = document.getElementById('vehOverlaySub');
const vehOverlayList = document.getElementById('vehOverlayList');
...
/* ---- Disturbances overlay ---- */
function openOverlay(backEl) {
  backEl.classList.add('open');
  backEl.setAttribute('aria-hidden', 'false');
  lockBodyScroll();
}
function closeOverlay(backEl) {
  if (!backEl.classList.contains('open')) return;
  backEl.classList.remove('open');
  backEl.setAttribute('aria-hidden', 'true');
  unlockBodyScroll();
}

distOverlayClose.addEventListener('click', () => closeOverlay(distOverlayBack));
distOverlayBack.addEventListener('mousedown', (e) => { if (e.target === distOverlayBack) closeOverlay(distOverlayBack); });

vehOverlayClose.addEventListener('click', () => closeOverlay(vehOverlayBack));
vehOverlayBack.addEventListener('mousedown', (e) => { if (e.target === vehOverlayBack) closeOverlay(vehOverlayBack); });

document.addEventListener('keydown', (e) => {
  if (e.key !== 'Escape') return;
  if (vehOverlayBack.classList.contains('open')) closeOverlay(vehOverlayBack);
  else if (distOverlayBack.classList.contains('open')) closeOverlay(distOverlayBack);
});
...
/* ---- Vehicle overlay ---- */
function mmToDelayPill(mins) {
  const m = Number(mins || 0);
  if (!isFinite(m) || m <= 0) return '';
  return '<span class="pill delay">+' + m + 'm</span>';
}

function fmtUnix(unixSeconds) {
  if (!unixSeconds) return '';
  return fmtTime(unixSeconds);
}

function renderVehicle(data) {
  vehOverlayList.innerHTML = '';

  const vehicleShort = (data.vehicleinfo && (data.vehicleinfo.shortname || data.vehicleinfo.name))
    ? (data.vehicleinfo.shortname || data.vehicleinfo.name)
    : (data.vehicle || 'vehicle');

  vehOverlayTitle.textContent = String(vehicleShort);

  const stops = (data.stops && Array.isArray(data.stops.stop)) ? data.stops.stop : [];
  if (!stops.length) {
    vehOverlaySub.textContent = 'No stops found.';
    return;
  }

  vehOverlaySub.innerHTML =
    '<div class="vehSummary">' +
      '<span class="pill">stops: ' + String(stops.length) + '</span>' +
      (data.timestamp ? '<span class="pill">updated: ' + escapeHtml(new Date(Number(data.timestamp) * 1000).toLocaleString()) + '</span>' : '') +
    '</div>';

  const list = document.createElement('div');
  list.className = 'vehStops';

  for (const s of stops) {
    const station = s.station || (s.stationinfo && (s.stationinfo.name || s.stationinfo.standardname)) || 'Unknown stop';

    const pl = (s.platform != null ? String(s.platform) : '?');

    // Scheduled times (what you asked for)
    const arrSched = s.scheduledArrivalTime ? fmtUnix(s.scheduledArrivalTime) : '';
    const depSched = s.scheduledDepartureTime ? fmtUnix(s.scheduledDepartureTime) : '';

    // Delays are in seconds
    const arrDelayM = Math.round((s.arrivalDelay || 0) / 60);
    const depDelayM = Math.round((s.departureDelay || 0) / 60);

    const arrCan = String(s.arrivalCanceled || '0') === '1';
    const depCan = String(s.departureCanceled || '0') === '1';

    const arrLine = (arrSched
      ? ('Arr ' + escapeHtml(arrSched) + (arrCan ? ' <span class="pill delay">cancelled</span>' : (arrDelayM > 0 ? ' ' + mmToDelayPill(arrDelayM) : '')))
      : '<span class="muted">Arr —</span>'
    );

    const depLine = (depSched
      ? ('Dep ' + escapeHtml(depSched) + (depCan ? ' <span class="pill delay">cancelled</span>' : (depDelayM > 0 ? ' ' + mmToDelayPill(depDelayM) : '')))
      : '<span class="muted">Dep —</span>'
    );

    const extraStop = String(s.isExtraStop || '0') === '1'
      ? '<span class="pill">extra stop</span>'
      : '';

    const occName = s.occupancy && (s.occupancy.name || s.occupancy['@id']) ? (s.occupancy.name || '') : 'unknown';
    const occ = normalizeOccName(occName);

    const row = document.createElement('div');
    row.className = 'vehRow';
    row.innerHTML =
      '<div class="vehTimes">' +
        '<div>' + arrLine + '</div>' +
        '<div class="tiny" style="margin-top:4px;">' + depLine + '</div>' +
      '</div>' +
      '<div>' +
        '<div class="vehStation">' + escapeHtml(station) + '</div>' +
        '<div class="vehSub">' +
          '<span class="pill ' + occ.cls + '">occupancy: ' + occ.label + '</span>' +
          (extraStop ? ' ' + extraStop : '') +
        '</div>' +
      '</div>' +
      '<div class="vehRight">' +
        '<div class="plMini"><div class="lbl">PLATFORM</div><div class="num">' + escapeHtml(pl) + '</div></div>' +
      '</div>';

    list.appendChild(row);
  }

  vehOverlayList.appendChild(list);
}

async function openVehicleOverlay(vehicleId, dateDDMMYY) {
  openOverlay(vehOverlayBack);
  vehOverlayTitle.textContent = 'Train details';
  vehOverlaySub.textContent = 'Loading…';
  vehOverlayList.innerHTML = '';

  try {
    let url = '/api/vehicle?id=' + encodeURIComponent(vehicleId) + '&lang=en&alerts=false';
    if (dateDDMMYY) url += '&date=' + encodeURIComponent(dateDDMMYY);

    const r = await fetch(url);
    const data = await r.json();
    if (!r.ok) throw new Error(data.error || 'Vehicle lookup failed');

    renderVehicle(data);
  } catch (e) {
    vehOverlaySub.textContent = 'Error';
    vehOverlayList.innerHTML = '<div class="muted">Error: ' + escapeHtml(e.message) + '</div>';
  }
}
...
searchBtn.addEventListener('click', async () => {
  try {
    ...
    html += '<div class="deps">';
    for (let i = 0; i < deps.slice(0, 24).length; i++) {
      const d = deps[i];
      ...
      const safeVeh = escapeHtml(vehicleId);
      ...
      html += '<div class="dep">'
        ...
        + '</div>'
        + '<div>'
        +   '<button class="toLink" type="button" data-veh="' + safeVeh + '">' + escapeHtml(to) + '</button>'
    ...
    // Bind vehicle overlay click handlers (destination button)
    const dateForVehicle = dateIRail || ''; // DDMMYY (or empty)
    const btns = board.querySelectorAll('.toLink');
    btns.forEach((b) => {
      b.addEventListener('click', () => {
        const veh = b.getAttribute('data-veh') || '';
        if (!veh) return;
        openVehicleOverlay(veh, dateForVehicle);
      });
    });
