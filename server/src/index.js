  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    @media (max-width:720px){
      ...
      .vehRow{grid-template-columns: 132px 1fr 84px;}
    }
  </style>
</head>
<body>
...
function renderVehicle(data) {
  ...
  for (const s of stops) {
    ...
    // Scheduled times (Arr + Dep)
    const arrSched = s.scheduledArrivalTime ? fmtTime(s.scheduledArrivalTime) : '';
    const depSched = s.scheduledDepartureTime ? fmtTime(s.scheduledDepartureTime) : '';

    // Delays (seconds -> minutes)
    const arrDelayM = Math.round((Number(s.arrivalDelay || 0)) / 60);
    const depDelayM = Math.round((Number(s.departureDelay || 0)) / 60);
...
searchBtn.addEventListener('click', async () => {
  try {
    ...
    let html = '<div class="headerline">'
      ...
      + '<span class="muted">updated: ' + new Date(Number(data.timestamp) * 1000).toLocaleString() + '</span>'
      + '</div>';
