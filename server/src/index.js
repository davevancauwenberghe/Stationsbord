...
app.get("/api/liveboard", async (req, res) => {
  try {
    const { id, station, date, time, arrdep = "departure", lang = "en", alerts = "false" } =
      req.query;
...
async function loadVehicleDetails(vehicleId) {
  ...
  for (const s of stops) {
    ...
    const depBadges = (depCan ? '<span class="miniPill miniDelay">cancelled</span>' : '') + delayMini(depDelay);
    const arrBadges = (arrCan ? '<span class="miniPill miniDelay">cancelled</span>' : '') + delayMini(arrDelay);
    ...
    html +=     '<div class="stopMeta" style="margin-top:6px;">' + '<span class="miniPill">' + arrLine + '</span>' + arrBadges + '</div>';
    html +=   '</div>';
