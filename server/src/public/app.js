...
(function () {
  ...
  function openRoutePlanner() {
    ...
    modalBody.innerHTML = '<div class="muted routeHint">' + escapeHtml(t("routeHint")) + '</div>' +
      ...
      '<label>' + escapeHtml(t("routeTimeMode")) + '<select id="routeTimesel" class="small"><option value="departure">' + escapeHtml(t("departure")) + '</option><option value="arrival">' + escapeHtml(t("arrival")) + '</option></select></label>' +
  ...
  async function searchConnections() {
    ...
    const timesel = timeselInput && timeselInput.value === 'arrival' ? 'arrival' : 'departure';
    out.innerHTML = '<div class="muted">' + escapeHtml(t("loading")) + '</div>';
    try {
      let url = '/api/connections?from=' + encodeURIComponent(connectionEndpointName(routeFrom)) + '&to=' + encodeURIComponent(connectionEndpointName(routeTo)) + '&lang=' + encodeURIComponent(getLanguage()) + '&timesel=' + encodeURIComponent(timesel) + '&typeOfTransport=automatic&results=6';
      if (dateIRail) url += '&date=' + encodeURIComponent(dateIRail);
